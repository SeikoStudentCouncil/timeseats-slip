...
\newcommand{\cell}[1]{%
  \begin{minipage}[t][\dimexpr(\paperheight - 5.1cm)/4 - 10pt\relax][t]{\dimexpr0.5\paperwidth -10pt -2mm\relax}
    #1
  \end{minipage}
}
...
\AddToShipoutPictureFG{
  \begin{tikzpicture}[remember picture, overlay, ampersand replacement=\&]
    ...
    \node[draw, circle, text=gray, line width=.5mm, minimum size=14mm, dashed] at ($($ (current page.north west) + (2mm,-1.5cm)$) !0.5! (id.west)$) {印};
    ...
    \matrix (matrix) [matrix of nodes, nodes={anchor=north west, align=left, inner sep=5pt, text width=\dimexpr0.5\paperwidth - 10pt -2mm\relax, draw},column sep=0pt, row sep=0pt, inner sep=0pt, outer sep=0pt, anchor=north west]
